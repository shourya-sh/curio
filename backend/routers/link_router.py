...
def list_links(session_id: str, db: Session = Depends(get_db), user_id: str = Depends(get_current_user)):
    ...
    return graph_service.list_links(db, sid)
...
def update_link(session_id: str, link_id: int, body: LinkUpdate, db: Session = Depends(get_db), user_id: str = Depends(get_current_user)):
    sid = resolve_session_pk_or_404(session_id, db)
    verify_session_owner(db, sid, user_id)
...
def delete_link(session_id: str, link_id: int, db: Session = Depends(get_db), user_id: str = Depends(get_current_user)):
    sid = resolve_session_pk_or_404(session_id, db)
    verify_session_owner(db, sid, user_id)
    if not graph_service.delete_link(db, sid, link_id):
        raise HTTPException(status_code=404, detail="Link not found")
    db.commit()
...
def restore_link(session_id: str, link_id: int, body: LinkRestorePayload, db: Session = Depends(get_db), user_id: str = Depends(get_current_user)):
    sid = resolve_session_pk_or_404(session_id, db)
    verify_session_owner(db, sid, user_id)
    if body.id != link_id:
        raise HTTPException(status_code=400, detail="Link id mismatch")
    try:
        link = graph_service.restore_link(db, sid, body.model_dump(exclude_none=True))
